...
var request = require('@wedo/request');
...
class CodeProjectRepo {
  ...
  async save(user) {
    ...
    for (const update of [..._findInstanceProperty(_context = this.project.getRoot()).call(_context, file => file.isDirty())]) {
      var _context;
      const result = await request.fileRemote.post1('codeless', update.getExt() || "", update.getContent() || "");
    ...
    await request.codeProjectRemote.put(user, this.project.getName(), this.project.toJSON());
    console.log('project saved.', this.project.toJSON());
  }
  static async load(user, name) {
    const result = await request.codeProjectRemote.get(user, name);
